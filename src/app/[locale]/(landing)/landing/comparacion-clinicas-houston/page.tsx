...
export default async function ComparacionLandingPage({
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-br from-green-deep via-green-dark to-green-primary py-20 text-mint-bg lg:py-28">
        <div
          aria-hidden
          className="pointer-events-none absolute -left-24 top-0 h-96 w-96 rounded-full bg-gold-accent/15 blur-3xl"
        />
      ...
      {/* Comparación */}
      <section className="bg-mint-warm py-20 lg:py-24">
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          <h2 className="text-center font-heading text-3xl font-extrabold tracking-tight text-slate-dark">
            {c.comparisonTitle}
          </h2>
          <Reveal className="mt-10 overflow-hidden rounded-2xl border border-green-light bg-white shadow-sm">
            <div className="grid grid-cols-[1fr_auto_auto] items-center gap-x-4 border-b border-green-light bg-mint-bg px-5 py-4 sm:gap-x-8 sm:px-8">
              <span className="text-xs font-semibold uppercase tracking-widest text-slate-muted">
                &nbsp;
              </span>
              <span className="text-center font-heading text-sm font-bold text-green-dark">
                {c.mangumLabel}
              </span>
              ...
              {c.comparison.map((row, i) => (
                ...
                >
                  ...
                  <span className="flex justify-center">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-green-light text-green-dark">
                      <Check className="h-4 w-4" />
                    </span>
                  </span>
                  <span className="flex justify-center">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-red-accent-bg text-red-accent">
                      <X className="h-4 w-4" />
                    </span>
                  </span>
                </li>
      ...
      {/* Diferenciadores */}
      <section className="bg-mint-bg py-20 lg:py-24">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {c.differentiators.map((d, i) => (
              <Reveal key={d.title} delay={i * 80}>
                <div className="h-full rounded-2xl border border-green-light bg-white p-7">
                  <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-green-primary font-heading text-lg font-extrabold text-white">
                    {i + 1}
                  </span>
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6">
          ...
          <ul className="mt-10 grid gap-3 sm:grid-cols-2">
            {c.offerings.map((o) => (
              <li
                key={o}
                className="flex items-start gap-3 rounded-xl border border-green-light bg-white px-4 py-3.5"
              >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {reviews.map((r) => (
              <figure
                key={r.author}
                className="flex h-full flex-col rounded-2xl border border-green-light bg-white p-6 shadow-sm"
              >
                ...
                <figcaption className="mt-5 border-t border-green-light pt-4 font-heading font-bold text-slate-dark">
                  {r.author}
                </figcaption>
              </figure>
            ))}
      ...
      <section
        id="lead-form"
        className="scroll-mt-20 bg-gradient-to-br from-green-deep via-green-dark to-green-deep py-20 text-mint-bg lg:py-24"
      >
