...
export default async function BlogPostPage({
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden bg-linear-to-br from-green-deep via-green-dark to-green-deep py-16 text-mint-bg lg:py-20">
        {post.hasCover ? (
          <>
            ...
            <div
              aria-hidden
              className="absolute inset-0 -z-10 bg-linear-to-r from-green-deep via-green-deep/85 to-green-deep/40"
            />
            <div
              aria-hidden
              className="absolute inset-0 -z-10 bg-green-deep/35"
            />
          </>
        ) : (
          <div
        ...
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <Link
            href="/blog"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-mint-bg/70 hover:text-white"
          >
          ...
          <p className="mt-4 text-lg leading-relaxed text-mint-bg/80">
            {post.description}
          </p>

          <div className="mt-6 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-mint-bg/70">
      ...
      {/* Contenido */}
      <section className="bg-mint-warm py-14 lg:py-20">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <article
            className={cn(
              "rounded-2xl border border-green-light bg-white px-6 py-8 shadow-sm sm:px-10 sm:py-10",
              "space-y-4",
              ...
              "[&_a]:text-green-primary [&_a]:underline [&_a]:underline-offset-2",
              "[&_table]:w-full [&_table]:border-collapse [&_table]:text-sm",
              "[&_th]:border [&_th]:border-green-light [&_th]:bg-mint-bg [&_th]:px-3 [&_th]:py-2 [&_th]:text-left [&_th]:font-semibold [&_th]:text-slate-dark",
              "[&_td]:border [&_td]:border-green-light [&_td]:px-3 [&_td]:py-2 [&_td]:align-top [&_td]:text-slate-primary",
            )}
          >
          ...
          {/* CTA */}
          <div className="mt-8 rounded-2xl bg-linear-to-br from-green-primary to-green-dark p-7 text-center text-white shadow-lg">
            <h2 className="font-heading text-xl font-bold">{t("ctaTitle")}</h2>
            <p className="mt-2 text-sm text-mint-bg/85">{t("ctaBody")}</p>
            <a
      ...
      {/* Relacionados */}
      {related.length > 0 && (
        <section className="bg-mint-bg py-14 lg:py-20">
